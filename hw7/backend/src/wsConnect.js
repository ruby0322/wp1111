...
export default {
  ...
  onMessage: (webSocketConnection, webSockectServer) => async byteString => {
    ...
    console.log(`Client '${webSocketConnection.username}' sent: ${byteString}.`);
    if (task === 'input') {
      const message = new Message(payload);
      await message.save();
    
      const group = await Group.findOne(payload.group.includes('_') ? { name: payload.group } : { _id: payload.group });

      group.members.forEach(
        member => {
          if (member !== payload.sender) {
            notify(
              ['output', {...payload, at: payload.group.includes('_') ? payload.sender : group.name}],
              member
            );
          } 
        }
      ...
      if (!existing) {
        const newUser = new User({ name: payload.username, password: payload.password, friends: [payload.username], groups: [] });
        await newUser.save();
      ...
      if (foundUser) {
        if (foundUser.friends.includes(webSocketConnection.username)) {
          ...
          const newPrivateGroup = new Group({
            name: getPrivateGroupName(webSocketConnection.username, foundUser.name),
      ...
      newGroup.members.forEach(
        async member => {
          ...
          await User.updateOne({ name: member }, {
            $push: {
              groups: {
                name: newGroup.name,
                id: newGroup._id
